...
parser.add_argument('--resume_path', type=str, default='./FAT_results/checkpoint.pth.tar', help='resume training file')
...
def train(model, train_loader, optimizer, tau):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output_adv, output_target, output_natural, count = earlystop(model, data, target, step_size=step_size,
                                                                     epsilon=epsilon, perturb_steps=num_steps, tau=tau,
                                                                     randominit_type="uniform_randominit", loss_fn='cent', rand_init=random, omega=omega)
        bp_count += count
        model.train()
        optimizer.zero_grad()
...
for epoch in range(start_epoch, args.epochs):
    ...
    loss, train_pgd20_acc = attack.eval_robust(model, train_loader, perturb_steps=20, epsilon=0.031, step_size=0.031 / 4,loss_fn="cent", category="Madry", rand_init=True)
