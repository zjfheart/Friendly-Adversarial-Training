...
parser.add_argument('--out-dir',type=str,default='./FAT_for_TRADES_results',help='dir of output')
parser.add_argument('--resume', type=bool, default=False, help='whether to resume training')
parser.add_argument('--resume-path', type=str, default='./FAT_for_TRADES_results/checkpoint.pth.tar', help='resume training file')
...
def train(model, train_loader, optimizer, tau):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output_adv, output_target, output_natural, count = earlystop(model, data, target, step_size=step_size,
                                                                     ...
                                                                     tau=tau, randominit_type="normal_distribution_randominit", loss_fn='kl', rand_init=random,
                                                                     omega=omega)
        bp_count += count
        model.train()
        optimizer.zero_grad()
        ...
        loss = TRADES_loss(adv_logits,natural_logits,output_target,beta)
