...
Results of TRADES (![](http://latex.codecogs.com/gif.latex?\beta=1.0) and ![](http://latex.codecogs.com/gif.latex?\beta=6.0)) are reported in [TRADES](https://arxiv.org/abs/1901.08573). FAT for TRADES has the same evaluations. Noted that our evaluations of the above are the same as the description in the TRADES's paper, i.e., adversarial data are generated without random start ```loss, test_pgd20_acc = attack.eval_robust(model,test_loader, perturb_steps=20, epsilon=0.031, step_size=0.003,loss_fn="cent",category="Madry",rand_init=False)```. 
However, in [TRADES’s experimental testing settings](https://github.com/yaodongyu/TRADES), they use random start before PGD perturbation that is deviated from the statements in their paper. For the evaluation of FAT with random start, these results refer to the Table 3 in [our paper](https://arxiv.org/pdf/2002.11242.pdf)
